...
import type { ContractRouter, RouteHandler as CoreRouteHandler, Router as CoreRouter } from '@ts-kizuna/core/adapter';

/**
 * The one definition every adapter's own `Router<C>` must resolve to, with its handler context substituted in.
 */
export type ExpectedRouter<C, HandlerContext> = C extends Contract
    ? ContractRouter<C, HandlerContext>
    : C extends Routes
