 ...
 */
const CONFIG_KEYS = ['adapter', 'tags', 'identities', 'requestContext', 'guardSchema', 'issueCodes', 'jobs', 'plugins'] as const;
...
export const generateConfigTypes = (configSource: string, fileName = 'kizuna.config.ts'): string => {
    ...
    for (const key of CONFIG_KEYS) {
        ...
        if (!value) continue;
        if (key === 'identities' || key === 'requestContext') {
            lines.push(`    ${key}: ${typeOfRecord(value, used, key, '    ')};`);
        } else if (key === 'issueCodes') {
            lines.push(`    ${key}: ${typeOfStringUnion(value, key)};`);
        } else if (key === 'plugins') {
