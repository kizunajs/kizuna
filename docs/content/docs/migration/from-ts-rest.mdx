...
ts-rest leaves authentication to your framework's middleware. Kizuna declares it: an [identity](/docs/reference/k-identity) says how the credential travels and what a passing guard provides, the guard sits on that identity, and a route's `auth` names the one it requires.
