...
The trap is the middle: a modest `maxAge` on content that changes at a stable URL. It looks reasonable and passes testing, then serves a stale response to somebody for as long as you set, with no way to recall it. [Jake Archibald's write-up](https://jakearchibald.com/2016/caching-best-practices/) is the canonical account of how that goes wrong.
