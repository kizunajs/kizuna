...
Caching is new and still settling, so `cache` and `etag` may change while v2 is in beta. A policy is also hard to take back once callers hold a response, so reach for it deliberately rather than by default.
