...
| `--types`  | beside the config  | Where `kizuna.types.ts` is written                                                      |
...
It carries `adapter`, `tags`, `identities`, `requestContext`, `guardSchema`, `issueCodes`, `jobs` and `plugins`. Your `routes` stay out: they are read at runtime, and typing against them would make the config a prerequisite for declaring the routes it serves.
