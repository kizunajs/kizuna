...
The handler's `query` and `body` are typed from the declaration right above it, and its return is checked against that route's `responses`. See [Handlers](/docs/handlers) for everything a handler receives.
...
- [Handlers](/docs/handlers) for everything a handler receives
- [Mounting](/docs/config) to assemble it and serve it
