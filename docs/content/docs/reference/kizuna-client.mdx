...
The response is a discriminated union over the route's declared status codes. Narrow on `status` to access the correctly typed `body`, or `stream` on a status declared with [`stream`](/docs/streaming).
