...
Resolvers run on every route, public ones included, before the guards, which read their values too, and never deny a request. They receive the adapter's native request objects, the route's `params`, and the declared `headers` (validated). The return is checked against the `context` schema, and `server.api` requires a resolver for every declared key. Request context never appears in the OpenAPI document.
