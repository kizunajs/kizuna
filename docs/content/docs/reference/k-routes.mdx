...
Use the `{ body, headers?, contentType?, cache?, etag? }` form of a response to declare [typed response headers](/docs/routes#response-schemas), a [binary body](/docs/reference/binary-and-file-schemas), or a [cache policy and entity tag](/docs/caching). Use `{ stream, headers?, contentType? }` to send the response piece by piece, see [Streaming](/docs/streaming).
