...
Use the `{ body, headers?, contentType?, cache?, etag? }` form of a response to declare [typed response headers](/docs/building/contract#response-schemas), a [binary body](/docs/reference/binary-and-file-schemas), or a [cache policy and entity tag](/docs/caching).
