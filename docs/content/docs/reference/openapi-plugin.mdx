...
Serve a reference UI for the OpenAPI document generated from your API, and the document itself if you want it public. Name it under `plugins` on [`defineConfig`](/docs/reference/define-config) and `api.mount` serves its routes. See the [OpenAPI guide](/docs/openapi).
...
These are [plugin routes](/docs/extend/create-plugin#plugin-routes): they live outside `api.routes`, so your fetch client, the generated Swift and Kotlin clients, and the document itself never list them.
