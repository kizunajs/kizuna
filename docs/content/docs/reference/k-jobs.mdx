...
Jobs are new and still settling. `k.jobs` may change before v2, so pin your version if you depend on it.

</AlphaNotice>

Declares the contract's scheduled jobs. The first argument is the identity every job requires, the credential your scheduler sends. Pass the result to `defineConfig` under `jobs`, alongside `routes`.
