...
The generated file also exports `API`, the namespace holding every model and every route's `Params`, `Query`, `Body` and `Result`. The config's `namespace` option calls it something else:
