...
description: Build and ship your own TypeScript client wrapper using the types and the route metadata @ts-kizuna/fetch carries.
---

<BetaNotice>

The wrapper surface is new and still settling. The types `@ts-kizuna/fetch` exports may change while v2 is in beta, so pin your version if you depend on it.

</BetaNotice>

A wrapper builds on the [fetch client](/docs/clients/fetch) your app already has, the one `kizuna generate` writes. Wrap it to build hooks, an SDK layer, or any other integration. `@ts-kizuna/tanstack-query` is one of these, and it takes the client and nothing else.
...
The types from `@ts-kizuna/fetch`:
