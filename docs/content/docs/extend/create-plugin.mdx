...
A plugin is always `server`: it is named on a config, and a config carries handlers, so it never reaches a browser. `kizuna.entries` is what this repo's boundary test reads, and it holds every package to the reach it declares.
