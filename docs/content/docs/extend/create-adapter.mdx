...
A `success` result on a status declared with [`stream`](/docs/streaming) renders with `body` undefined and a `stream` function in its place. Check `stream` before `body`, hand it the signal that fires when the client goes away, and write the `ReadableStream` it returns. `encodeStream(result, { signal })` does the same for an adapter that renders results itself.
