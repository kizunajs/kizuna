...
The return is checked against the identity's schemas. `deny(status, detail, headers?)` responds with RFC 9457 Problem Details. A `401` also carries the `WWW-Authenticate` challenge RFC 9110 requires, named after the identity that denied: `Bearer`, or `Basic` for a basic one. An API key is not HTTP authentication and has no challenge to send, so `403` is the honest refusal for one. Guards also get the adapter's native request objects, oauth2 `scopes`, and whatever [request context](/docs/request-context) the contract declares, resolved before the guard runs. See [`server.guard`](/docs/reference/server-guard). For request-scoped values that gate nothing (analytics ids, loggers), declare a request context rather than an identity.
