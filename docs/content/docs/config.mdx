...
| Key              | What it brings                                                                                                                                                                                              |
| ---------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `auth`           | The identities a route's `auth` can require. Each carries its own guard, so this is all the config needs. An identity used without a guard refuses to assemble. See [Authentication](/docs/authentication). |
...
```ts title="src/app/api/[...kizuna]/route.ts"
import kizuna from '../../../../kizuna.config';
...
Each client comes from the package that generates it, so its options are typed there and a generator you have not installed cannot be configured by accident. The list is not closed: any package exporting a [`ClientTarget`](/docs/extend/create-generator#shipping-a-clienttarget) factory drops into `clients` the same way.
...
The generated client holds the route shapes and the schemas, and is what a browser imports. An `output` is resolved against the config's own directory, so in a repository holding both, `../ios/MyApp/Generated/APIClient.swift` writes into the app beside it.
...
| `namespace`           | Fetch, Swift, Kotlin | Name of the generated namespace, `API` by default                         |
...
Each config declares its own clients, so the Swift and Kotlin clients for the admin API come out of `kizuna.admin.config.ts` along with its types.
...
<InstallTabs packageName="@kizunajs/cli" dev />

```sh title="Terminal"
kizuna generate
```

It writes three things from `kizuna.config.ts`: the `Config` in `kizuna.types.ts`, every client under `clients`, and the `.kizuna/snapshot.yaml` that [`kizuna diff`](/docs/breaking-changes) compares. A file that already matches is left alone, so watchers and build tools see no change.
...
The same work without writing. It names whatever has fallen behind and exits `1`, so a pipeline fails on a file nobody regenerated:

```text
2 generated files are out of date:
  src/lib/api-client.generated.ts is behind the config
  .kizuna/snapshot.yaml is behind the config
...
Run it in CI, where [the CLI page](/docs/cli#kizuna-generate) has the workflow. [`kizuna diff`](/docs/breaking-changes) reads the snapshot this writes, so the two belong in the same pipeline: the check proves the snapshot matches today's config, and the diff compares it against the base.
