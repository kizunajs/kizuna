...
It writes three things from `kizuna.config.ts`: the `Config` in `kizuna.types.ts`, every client under `clients`, and the `.kizuna/diff.yaml` snapshot [`kizuna diff`](/docs/breaking-changes) compares. A file that already matches is left alone, so watchers and build tools see no change.
...
2 generated files are out of date:
  ...
  .kizuna/diff.yaml is behind the config
