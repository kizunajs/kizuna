...
Split when they are deployed apart. A config names one `adapter`, so an API on Express and an API on Next.js are already two configs.
...
Stay with one config when the routes differ only by prefix. A `/admin` group under the same identities is a [route group](/docs/routes), not a second API, and splitting it costs you the shared types.
...
```ts title="kizuna.admin.config.ts"
import { defineConfig } from '@ts-kizuna/core';
import { expressAdapter } from '@ts-kizuna/express';
...
A route declared against the admin `k` names the admin identities and nothing else, so naming a public identity on an admin route is a type error rather than a runtime surprise.
...
```sh title="Terminal"
ts-kizuna-swift generate --config kizuna.admin.config.ts --output ... --namespace-name AdminAPI
```

### Sharing between them

What both APIs need lives in a module they both import: schemas, [models](/docs/reference/kizuna-model), an identity, or a whole route group. `Kizuna.model` keeps a shared schema's name, so `User` is one named type in both generated clients rather than two shapes that happen to match.
