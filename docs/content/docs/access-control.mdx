...
The least settled part of kizuna. `Kizuna.roles` and `Kizuna.permissions` may change shape or be removed while v2 is in beta. Pin your version, and expect to rewrite what you build on them.
...
An identity with a `context` and nothing else. Its guard, which you attach below, says who the caller is, and every route under it is open to every caller it accepts:
...
export const projects = k.routes('projects', {
    listProjects: k
        .route({
            method: 'GET',
            path: '/projects',
...
export const member = k.identity
    ...
    .guard(async ({ apiKey, params, deny }) => {
...
```ts title="src/db/with-workspace.ts"
...
`defineConfig` puts `401` and `403` on every route this map guards, as [Problem Details](/docs/reference/problem-details-schema), both `no-store`. A public route gets neither:
