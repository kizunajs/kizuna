...
`@ts-kizuna/next` serves a ts-kizuna API from a Next.js App Router application, through a single catch-all route file.
...
| `onError`            | none    | Map a thrown error into a response (inbound migration seam); return an `AdapterResult`/`Response` to override the default 500. Also settable on `defineConfig`. |

## `onError`

Set `onError` on `defineConfig`, or pass it to `mount`, to send your own response:
