...
A route the contract's `auth` map guards carries `401` and `403` in its response type, as [Problem Details](/docs/reference/problem-details-schema). A public route carries neither. A route declaring its own `403` keeps that body alongside, the way a declared `400` sits beside the validation error.
