...
On a status the route declares with [`stream`](/docs/streaming), the result carries `stream`, an async iterable, in place of `body`. Read it with `for await`, and each message narrows on `event` to its own `data`:
