...
When the contract secures routes with [identities](/docs/authentication), send the credential in `baseHeaders`. The server's guards read it, and answer `401`/`403` when it's missing or insufficient:
...
A route the contract's access control map guards carries `401` and `403` in its response type, as [Problem Details](/docs/reference/problem-details-schema). A public route carries neither. A route declaring its own `403` keeps that body alongside, the way a declared `400` sits beside the validation error.
