---
title: Swift
description: Generate a native Swift client from your routes, using URLSession and Codable.
---
...
<InstallTabs packageName="@kizunajs/swift" />
...
The path is relative to the config, so a client can land outside the TypeScript project. When the iOS app lives in the same repository, `../ios/MyApp/Generated/APIClient.swift` writes straight into it. An app in its own repository takes the generated file the way it takes any other vendored source, by copying it in from a build step or committing it.
...
Request inputs are passed as components named after the route's input groups: `.params` (path), `.body`, `.query`, and `.headers`:
...
A route whose response [streams](/docs/streaming) returns its `body` as an `AsyncThrowingStream`. Named events become an `Event` enum with one case per event, each carrying its decoded payload:
...
`kizuna generate` writes every client the config declares, so the Swift one is never generated on its own:
...
Run [`kizuna generate --check`](/docs/config#kizuna-generate---check) in CI, and a Swift client nobody regenerated fails the build instead of drifting from the routes it was generated from.
