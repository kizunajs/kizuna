...
| Flag                  | Description                                                                                                                       |
| --------------------- | --------------------------------------------------------------------------------------------------------------------------------- |
| `--config`            | Path to your `kizuna.config.ts`. Suffix it with an export to read something other than the default, as in `src/apis.ts:workspace` |
...
| `--export`            | Export name to read when none is suffixed. Default: `api`                                                                         |

The CLI loads the config at runtime using `jiti`.

Most projects name the generator under `clients` in the config instead and run [`kizuna generate`](/docs/configuration), which writes every client in one command.
...
{
    "scripts": {
        "generate:swift": "ts-kizuna-swift generate --config kizuna.config.ts --output ../ios/MyApp/Generated/APIClient.swift --namespace-name API"
    }
}
```
