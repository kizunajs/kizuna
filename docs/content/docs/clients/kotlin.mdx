...
A route whose response [streams](/docs/streaming) returns its `stream` as a `Flow`. Named events become a sealed `Event` interface with one class per event, each carrying its decoded payload as `data`:
