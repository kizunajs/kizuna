...
<InstallTabs packageName="@kizunajs/kotlin" />
...
The path is relative to the config, so a client can land outside the TypeScript project. When the Android app lives in the same repository, `../android/app/src/main/kotlin/com/example/APIClient.kt` writes straight into it. An app in its own repository takes the generated file the way it takes any other vendored source, by copying it in from a build step or committing it.
...
`kizuna generate` writes every client the config declares, so the Kotlin one is never generated on its own:
...
Run [`kizuna generate --check`](/docs/config#kizuna-generate---check) in CI, and a Kotlin client nobody regenerated fails the build instead of drifting from the routes it was generated from.
