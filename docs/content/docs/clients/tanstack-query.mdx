...
`@ts-kizuna/tanstack-query` turns your api and a [fetch client](/docs/clients/fetch) into TanStack Query options. Query keys come from each route's own path, so invalidation is a method call.
