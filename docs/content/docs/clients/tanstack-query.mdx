...
description: Build TanStack Query options from your routes, with query keys derived from your routes.
---
...
`@kizunajs/tanstack-query` turns your api and a [fetch client](/docs/clients/fetch) into TanStack Query options. Query keys come from each route's own path, so invalidation is a method call.
...
<InstallTabs packageName="@kizunajs/fetch @kizunajs/tanstack-query @tanstack/react-query" />

</Tab>

<Tab value="Vue">

<InstallTabs packageName="@kizunajs/fetch @kizunajs/tanstack-query @tanstack/vue-query" />

</Tab>

<Tab value="Svelte">

<InstallTabs packageName="@kizunajs/fetch @kizunajs/tanstack-query @tanstack/svelte-query" />
...
It takes the [fetch client](/docs/clients/fetch) you already have:
...
Every client method carries the route it answers, so the client is the only argument.
...
A route whose response [streams](/docs/streaming) offers `streamOptions` in place of `queryOptions`. It builds on TanStack's `experimental_streamedQuery`, so `data` is the list of messages received so far and grows as they arrive:
