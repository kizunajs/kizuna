...
- `tools.definitions` is every tool in MCP's shape, `inputSchema` as JSON Schema.
- `tools.keyOf` turns `weather_get_forecast` into the dotted key.
- `tools.call` validates the input, runs the handler, validates the output, and answers the `tool_result` payload.
